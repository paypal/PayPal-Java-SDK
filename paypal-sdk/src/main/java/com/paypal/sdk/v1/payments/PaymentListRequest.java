// This class was generated on Thu, 29 Mar 2018 19:06:15 UTC by version 0.1.0-dev+2136c8 of Braintree SDK Generator
// PaymentListRequest.java
// @version 0.1.0-dev+2136c8
// @type request
// @data 
// DO NOT EDIT
package com.paypal.sdk.v1.payments;

import com.braintreepayments.http.*;
import java.util.*;
import java.util.stream.Collectors;
import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;
/**
 * Lists payments that were created by the [create payment](#payment_create) call and that are in any state. The list shows the payments that are made to the merchant who makes the call. To filter the payments that appear in the response, you can specify one or more optional query and pagination parameters. See [Filtering and pagination](/docs/api/overview/#query-parameters).
 */
public class PaymentListRequest extends HttpRequest<PaymentHistory> {

    public PaymentListRequest() {
        super("/v1/payments/payment?", "GET", PaymentHistory.class);
        
        header("Content-Type", "application/json");
    }

    public PaymentListRequest count(Integer count) {
        String params = String.valueOf(count);
        try {
		    path(path() + "count=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest endTime(String endTime) {
        String params = String.valueOf(endTime);
        try {
		    path(path() + "end_time=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest payeeId(String payeeId) {
        String params = String.valueOf(payeeId);
        try {
		    path(path() + "payee_id=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest sortBy(String sortBy) {
        String params = String.valueOf(sortBy);
        try {
		    path(path() + "sort_by=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest sortOrder(String sortOrder) {
        String params = String.valueOf(sortOrder);
        try {
		    path(path() + "sort_order=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest startId(String startId) {
        String params = String.valueOf(startId);
        try {
		    path(path() + "start_id=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest startIndex(Integer startIndex) {
        String params = String.valueOf(startIndex);
        try {
		    path(path() + "start_index=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }

    public PaymentListRequest startTime(String startTime) {
        String params = String.valueOf(startTime);
        try {
		    path(path() + "start_time=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }
    
}
